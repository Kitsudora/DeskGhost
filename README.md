...
面向 Windows 的桌宠与任务图工具，使用 **Electron + HTML / CSS / Canvas**，没有传统 Windows 标题栏或边框。v0.5.8 恢复取消创建时从卡片表面向四周散开的粒子动画，刚召唤或完成组装后取消均可退场；修正启动数据较晚返回时，重新打开已被 Esc 收起的拉栏、导致显示状态与窗口焦点不一致的问题。保留 v0.5.7 的任务图底部显示区域和首次快捷键置顶修复，以及魔杖形状容错、粒子聚合成卡片、焦点切换与保存保护。场景外保持透明，界面使用英文。
...
本轮体验包使用 `artifacts/cancel-escape-0.5.8/DeskGhost-win32-x64/`，与已有版本分开保存。创建独立发布目录可执行：

```powershell
node scripts/build-desktop.cjs --package --out artifacts/cancel-escape-0.5.8
```
...
| 浏览时间列 / 缩放 | 按住鼠标中键拖动画面，或用 `Shift` + 滚轮左右移动；`Ctrl` + 滚轮或 `+` / `-` 缩放，上限为原尺寸，下限为纵向容纳五张卡片（包含间隔）。最高的任务堆叠超出可视高度后，普通滚轮才允许上下移动 |
...
新卡片创建后可直接拖线接到已有任务之后。连接及其引起的移动、追加时间列及移入，均可一次撤销或重做。断开、改接、移动和删除也可撤销；会形成循环或超过上限的操作会被拒绝。图内拖动到边缘时可继续浏览时间列，纵向移动仍受任务堆叠高度限制。动画和过渡不承担保存或输入时序，关闭效果后仍可创建、编辑和使用快捷键。
